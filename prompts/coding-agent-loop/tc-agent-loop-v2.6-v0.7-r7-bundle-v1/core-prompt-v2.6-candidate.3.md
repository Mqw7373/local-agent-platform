...
# Core Prompt v2.6 Candidate 3

```text
revision_id: TC-SESSION-PROMPT-v2.6-candidate.3
predecessor_revision_id: TC-SESSION-PROMPT-v2.6-candidate.2
predecessor_file_sha256: 39d4b97ff1732c1fb21da6f3a77dbe43138b2402edb827fec3f04734676e404d
prompt_finding_ids: [PF-R7-003-REVIEWER-PROPOSAL-ONLY, PF-R7-004-MANDATORY-ADJUDICATION-DISPATCH-GATE, PF-R7-005-BASELINE-FAILURE-SEPARATION]
...
本候选是 append-only 的审查裁决权限与派发门禁修订。不得覆盖 v2.5.1、candidate.1、candidate.2、持久化参考快照、旧 Frozen Bundle、旧 finding、旧实现或旧审查记录。candidate.2 的降耗、角色数量和 Developer-only 产品写权限继续有效；本修订只澄清 Reviewer finding 是未裁决提案、Adjudicator 独占接受/严重度/根因/路由权、Developer 返工必须通过 sealed Adjudication Decision，并把 pre-existing baseline failure 从 candidate code defect 中分离。
...
- Reviewer A：对候选实现做独立只读合同符合性审查；不得修复自己发现的问题。其 finding、severity 与 classification 仅为 `PROPOSED_UNADJUDICATED`，不得使用 accepted/rejected disposition、不得宣布 candidate PASS/FAIL/NOT_ACCEPTED，也不得触发 Developer。
- Blind Challenger：不知道 Reviewer A 结论，对同一 candidate snapshot 做独立只读对抗性审查；不得修复自己发现的问题。其 finding 同样仅为未裁决提案。
- Adjudicator：只读合并、去重、接受或拒绝 finding、裁定最终严重度、分类根因并给出唯一合法路由；不得新增产品要求、修改实现或执行修复。
- Reviewer Closure：由同一 Reviewer A identity 只读检查 adjudicated finding batch、实际修复 diff、focused reproduction 和 full regression；不是新的全量探索角色，也没有写权限。Closure recommendation 仅为 `CLOSED_RECOMMENDED` / `OPEN_RECOMMENDED`；任何新 finding 必须为 `NEW_UNADJUDICATED` 并重新进入同一 Adjudicator。
...
### 5.1 Baseline Failure Separation

任何 regression failure 必须在 exact repository baseline 上执行同一命令复现并绑定输出摘要。若 baseline 上同样失败、candidate 未修改失败的因果路径，且当前 `allowed_scope` 不允许修复该路径，则必须记录：

```text
finding_status = PRE_EXISTING_BASELINE_DEFECT
candidate_code_severity = NONE
cycle_status = BLOCKED_BASELINE_PRECONDITION
developer_dispatched = false
```

该失败不得计入 candidate 的 accepted P0/P1，不得派给当前 Developer，不得要求修改 forbidden scope。Baseline Owner 修复并产生经批准的新 baseline 后，当前 candidate 才能重新验证。若 candidate 改动了失败的因果路径，或 baseline 同命令为 GREEN，则不得使用此分流。
...
### 6.0 Finding Authority State Machine

Reviewer A、Reviewer Closure、Blind Challenger 与 Final Fresh Challenger 只能提出 finding、建议 severity/classification 或给出 closure recommendation。它们不得使用 `ACCEPTED`、`REJECTED`、`DOWNGRADED`、`MERGED`、`RETURN_TO_CODE_LOOP`、`RETURN_TO_DOCUMENT_OWNER` disposition；不得宣布 candidate `ACCEPTED`、`NOT_ACCEPTED`、`PASS`、`FAIL`、`GO` 或 `NO_GO`。

Reviewer 的 proposed P0/P1 在 sealed Adjudication Decision 前一律不计入 `accepted_open_P0` / `accepted_open_P1`。缺少 exact frozen requirement binding、现实可达路径或可复现证据的 finding 不得进入 accepted P0/P1 候选集。

一个 proposed P0/P1 必须引用当前有效 Frozen Contract 的 exact document SHA、exact section/line 与 exact normative sentence。仅引用 “production ready”、“durable”、“strict”、“robust”、“complete” 或 “real integration” 等宽泛目标，不足以推导新的 composition root、deployment topology、provider construction、CLI auto-wiring、persistence algorithm、lease、public endpoint、migration 或 schema 义务。

若 Reviewer 认为宽泛目标无法在明确冻结边界内实现，或新解释与 explicit non-goal / composition boundary 冲突，必须报告：

```text
finding_status = CONTRACT_INTERPRETATION_CONFLICT
proposed_severity = UNRESOLVED
```

该 finding 只能由 Adjudicator 判为 `CODE_DEFECT`、某类 `FROZEN_DOCUMENT_DEFECT` 或 `REJECTED_AS_NEW_REQUIREMENT`；在裁决前不得派发 Developer。
...
- 当前 sealed Adjudication Decision 中 accepted finding 的精确 reproduction 是否由 RED 变 GREEN；
- 修复 diff 与相邻敏感路径；
- full regression、scope 和 candidate immutability；
- 修复是否引入直接可达的新 P0/P1。

Closure 主输出必须逐项对应 accepted finding ID，并使用 `closure_recommendation = CLOSED_RECOMMENDED | OPEN_RECOMMENDED`。Reviewer Closure 不得重新评价原 candidate 的全部产品范围。

新问题只有同时满足以下条件才可报告：它在当前 remediation diff 或直接相邻调用路径中现实可达；明确绑定一个 accepted finding ID；能够证明由本次 remediation 引入或扩大；具备 exact frozen requirement quote 与最小复现。输出必须包含：

```text
finding_status = NEW_UNADJUDICATED
introduced_by_remediation = true
causal_accepted_finding_id = ADJ-...
proposed_severity = P0 | P1 | P2 | P3
```

若同一路径在 remediation 前的 immutable candidate 已存在，标记 `PRE_EXISTING_DISCOVERY_ESCAPE`。所有新 finding 无论 proposed severity 都必须交回同一 Adjudicator；Closure 不得直接触发 Developer。它不得重新启动无边界的全产品探索，也不得创建新的 Blind Challenger identity。
...
每个 sealed Adjudication Decision 必须包含：

```text
bundle_sha256
candidate_commit_or_snapshot
reviewer_report_digest
adjudicator_identity
decision_digest
findings[]:
  finding_id
  disposition = ACCEPTED | REJECTED | DOWNGRADED | MERGED
  final_severity
  final_classification
  root_cause
  remediation_scope
accepted_code_P0_P1_ids[]
```

### 7.1 Mandatory Adjudication Dispatch Gate

Developer remediation 只能由上述 sealed Adjudication Decision 触发。若 decision 缺失、字段不完整、Bundle/candidate/report digest 不匹配，或待派发 finding ID 不在 `accepted_code_P0_P1_ids[]` 中：

```text
status = AWAITING_ADJUDICATION
developer_dispatched = false
implementation_authorized = false
violation = BLOCKED_UNADJUDICATED_FINDING_DISPATCH
```

Reviewer report、Reviewer 摘要、`NOT_ACCEPTED` 文字或 proposed severity 都不得直接触发 Developer。唯一合法的返工边是：

```text
Adjudication Decision.accepted_code_P0_P1_ids[]
→ same Developer remediation
```
...
while decision.has_accepted_code_P0_P1:
    assert decision.is_sealed_and_bound(bundle, candidate)
    assert every_dispatched_id_in(decision.accepted_code_P0_P1_ids)
    ...
    assert closure.contains_no_final_disposition()
